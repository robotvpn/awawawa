...
username=$1
password=$2
user_id=$(mysql -h$HOST -P$PORT -u$USER -p$PASS $DB -sN -e "select user_name from users where user_name = '$username' AND user_pass = '$password' AND user_validated=1 AND  users.duration > 0  AND users.frozen =0 ")   
##Check user
