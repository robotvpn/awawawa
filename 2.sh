...
username=$1
password=$2
user_id=$(mysql -h$HOST -P$PORT -u$USER -p$PASS $DB -sN -e "select user_name from users where user_name = '$username' AND user_pass = '$password' AND user_validated=1 AND  users.vip_duration > 0  AND users.frozen =0 ")   
##Check user
...
echo -e "$GREEN                                         BY:Genasky of Leakteam  $RESET"
#!/bin/bash
################################################
#-----------------------
